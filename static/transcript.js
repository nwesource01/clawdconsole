...
(() => {
  ...
  // Tags (tri-state like Docs)
  const tagsEl = document.getElementById('t_tags');
  const tagsClearBtn = document.getElementById('t_tags_clear');
  const tagsManageBtn = document.getElementById('t_tags_manage');
  const TAGS_LS_KEY = 'claw_transcript_tag_state_v1';
  let allTags = []; // {id,title}
  let tagState = {};
  function triNext(st){
  ...
  async function loadTags(){
    ...
    try {
      ...
      const known = new Set(allTags.map(t => String(t && t.id || '')));
  ...
  function renderTagFilters(){
    ...
    tagsEl.innerHTML = tags.map(t => {
      ...
      return '<button type="button" class="pill ' + esc(st) + '" data-tag="' + esc(id) + '" data-state="' + esc(st) + '">' + esc(title) + '</button>';
    }).join('');
    ...
    if (tagsManageBtn) {
      tagsManageBtn.onclick = () => openManageDialog();
    }
  }
  ...
  function ensureManageDialog(){
    ...
    host.innerHTML = ''
      ...
      +     '<button id="tagManageClose" class="tbtn" type="button">Close</button>'
      +   '</div>'
      ...
      +       '<button id="tagManageCreate" class="tbtn" type="button">+ Tag</button>'
      +     '</div>'
  ...
  async function openManageDialog(){
    ...
    async function reloadTags(){
      await loadTags();
      const custom = (allTags||[]).filter(t => String(t.source||'manual') === 'manual');
      const auto = (allTags||[]).filter(t => String(t.source||'manual') !== 'manual');
  ...
  async function fetchPage({ reset } = {}) {
    ...
    const ins = inTags();
    const outs = outTags();
    if (ins.length) u.searchParams.set('tagIn', ins.join(','));
    if (outs.length) u.searchParams.set('tagOut', outs.join(','));
