...
(() => {
  ...
  // Chat tag controls
  const chatTagsBar = document.getElementById('chatTagsBar');
  const chatTagsEl = document.getElementById('chatTags');
  const chatTagManage = document.getElementById('chatTagManage');
  const chatTagClear = document.getElementById('chatTagClear');
  const chatTagMsg = document.getElementById('chatTagMsg');
  ...
  // --- Project tags (tri-state) ---
  const TAG_STATE_LS_KEY = 'cc_chat_tag_state_v1';
  let TAGS = []; // {id,title,source,workspaceId}
  let tagState = {}; // id -> off|in|out
  ...
  function loadTagState(){
    try {
      const raw = localStorage.getItem(TAG_STATE_LS_KEY);
      if (!raw) return {};
  ...
  function saveTagState(){
    try { localStorage.setItem(TAG_STATE_LS_KEY, JSON.stringify(tagState||{})); } catch {}
  }

  function inTags(){
    return Object.entries(tagState||{}).filter(([,v]) => v==='in').map(([k]) => k);
  }
  function outTags(){
    return Object.entries(tagState||{}).filter(([,v]) => v==='out').map(([k]) => k);
  }

  function setTagMsg(t){ try { if (chatTagMsg) chatTagMsg.textContent = String(t||''); } catch {} }
  ...
  const identityFleetRole = document.getElementById('identityFleetRole');
  const identityHostNotes = document.getElementById('identityHostNotes');
  ...
  async function identityLoad(){
    try { if (identityMsg) identityMsg.textContent = 'Loading…'; } catch {}
    try {
      const res = await fetch(apiUrl('/api/ops/brand'), { credentials:'include', cache:'no-store' });
      const j = await res.json();
      if (!res.ok || !j || !j.ok) throw new Error('http ' + res.status);
      const b = j.brand || {};
      if (identityName) identityName.value = String(b.assistantName || '');
      if (identityEmail) identityEmail.value = String(b.assistantEmail || '');
      if (identityPhone) identityPhone.value = String(b.phone || '');
      if (identityProvider) identityProvider.value = String(b.hostingProvider || '');
      if (identityFleetRole) identityFleetRole.value = String(b.fleetRoleLabel || '');
      if (identityHostNotes) identityHostNotes.value = String(b.hostNotes || '');
      try { if (identityMsg) identityMsg.textContent = ''; } catch {}
      return b;
    } catch {
      try { if (identityMsg) identityMsg.textContent = 'Load failed.'; } catch {}
      return null;
    }
  }

  async function identitySaveFn(){
    try { if (identityMsg) identityMsg.textContent = 'Saving…'; } catch {}
    try {
      const body = {
        assistantName: identityName ? String(identityName.value||'') : '',
        assistantEmail: identityEmail ? String(identityEmail.value||'') : '',
        phone: identityPhone ? String(identityPhone.value||'') : '',
        hostingProvider: identityProvider ? String(identityProvider.value||'') : '',
        fleetRoleLabel: identityFleetRole ? String(identityFleetRole.value||'') : '',
        hostNotes: identityHostNotes ? String(identityHostNotes.value||'') : '',
      };
      const res = await fetch(apiUrl('/api/ops/brand'), {
        method:'POST',
      ...
      if (!res.ok || !j || !j.ok) throw new Error('http ' + res.status);
      try { if (identityMsg) identityMsg.textContent = 'Saved.'; } catch {}
      try { await loadBrand(); } catch {}
      setTimeout(() => { try { window.location.reload(); } catch {} }, 350);
    } catch {
      try { if (identityMsg) identityMsg.textContent = 'Save failed.'; } catch {}
    }
  }

  if (identityClose) identityClose.addEventListener('click', identityCloseFn);
  if (identityModal) identityModal.addEventListener('click', (e) => { if (e && e.target === identityModal) identityCloseFn(); });
  if (identitySave) identitySave.addEventListener('click', identitySaveFn);

  document.addEventListener('click', (e) => {
    const t = e && e.target;
    if (!t) return;
    if (t && t.id === 'identityEdit') {
      identityOpen();
      identityLoad();
    }
  });
  ...
  async function loadTags(){
    ...
    try {
      ...
      // Ensure state only references known tags.
      const known = new Set(TAGS.map(t => String(t && t.id || '')));
      for (const k of Object.keys(tagState||{})) {
        if (!known.has(k)) delete tagState[k];
      }
      saveTagState();
    } catch (e) {
      TAGS = [];
      setTagMsg('Tags load failed');
    }
    renderTagButtons();
  }

  function renderTagButtons(){
    if (!chatTagsEl) return;
    const tags = (TAGS || []).slice().sort((a,b) => String(a.title||a.id||'').localeCompare(String(b.title||b.id||'')));
    chatTagsEl.innerHTML = tags.map(t => {
      const id = String(t.id||'');
      const title = String(t.title||id||'');
      const st = (tagState && tagState[id]) ? tagState[id] : 'off';
      const safeTitle = esc(title);
      const safeId = esc(id);
      const tip = (st === 'in') ? 'IN (tag outgoing + include filter)' : (st === 'out') ? 'OUT (exclude filter)' : 'OFF';
      return '<button type="button" class="pill ' + esc(st) + '" data-tag="' + safeId + '" data-state="' + esc(st) + '" title="' + esc(tip) + '">' + safeTitle + '</button>';
    }).join('');

    Array.from(chatTagsEl.querySelectorAll('button[data-tag]')).forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        saveTagState();
        renderTagButtons();
        // Re-render chat with new filters.
        try { renderMessages(messageCache); } catch {}
      });
    });

    // Clear button
    if (chatTagClear) {
      chatTagClear.onclick = () => {
        tagState = {};
        saveTagState();
        renderTagButtons();
        try { renderMessages(messageCache); } catch {}
  ...
  function renderMessages(msgs) {
    ...
    const ins = inTags();
    const outs = outTags();
    const filtered = (Array.isArray(msgs) ? msgs : []).filter(m => {
      const g = Array.isArray(m && m.tags) ? m.tags.map(x => String(x||'').trim()).filter(Boolean) : [];
  ...
  function openPmModalFromMessage(m){
    ...
    const pmTagPick = document.getElementById('pm_tag_pick');
    const pmTagManage = document.getElementById('pm_tag_manage');
    ...
    // Tag picker: default to the source message tags, else current IN tags
    const msgTags = Array.isArray(m && m.tags) ? m.tags.map(x => String(x||'').trim()).filter(Boolean) : [];
    const selected = msgTags.length ? msgTags : inTags();
    try { window.__pmDraftTags = selected.slice(0, 12); } catch { window.__pmDraftTags = selected.slice(0, 12); }

    function renderPmTagPick(){
      if (!pmTagPick) return;
      const sel = Array.isArray(window.__pmDraftTags) ? window.__pmDraftTags : [];
      const tags = (TAGS||[]).slice().sort((a,b) => String(a.title||a.id||'').localeCompare(String(b.title||b.id||'')));
      pmTagPick.innerHTML = tags.map(t => {
        const id = String(t.id||'');
        const title = String(t.title||id||'');
        const on = sel.includes(id);
        // reuse pill styles: in = selected
        return '<button type="button" class="pill ' + (on ? 'in' : 'off') + '" data-pmtag="' + esc(id) + '">' + esc(title) + '</button>';
      }).join('');
      Array.from(pmTagPick.querySelectorAll('button[data-pmtag]')).forEach(b => {
        b.addEventListener('click', () => {
          const id = b.getAttribute('data-pmtag') || '';
          const cur = Array.isArray(window.__pmDraftTags) ? window.__pmDraftTags : [];
          const next = cur.includes(id) ? cur.filter(x => x !== id) : cur.concat([id]);
          window.__pmDraftTags = next.slice(0, 12);
          renderPmTagPick();
        });
      });
    }
    ...
    if (pmTagManage) {
      pmTagManage.onclick = async () => {
        try {
          // uses the shared manage dialog wired to chatTagManage
          if (typeof openManageDialog === 'function') await openManageDialog();
        } catch {}
        try { await loadTags(); } catch {}
        renderPmTagPick();
      };
    }

    // ensure tags loaded before rendering
    Promise.resolve(loadTags()).finally(() => renderPmTagPick());
  ...
  async function savePmCard(){
    ...
    try {
      ...
      const tags = Array.isArray(window.__pmDraftTags)
        ? window.__pmDraftTags.map(x => String(x||'').trim()).filter(Boolean).slice(0, 12)
        : [];
      col.cards.push({
        id: newId,
        title,
  ...
  async function postMessage(extra) {
    ...
    const res = await fetch(apiUrl('/api/message'), {
      ...
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, attachments: atts, tagIds: inTags(), ...(extra || {}) }),
    });
  ...
  function ensureManageDialog(){
    ...
    host.innerHTML = ''
      ...
      +     '<button id="tagManageClose" class="pill" type="button">Close</button>'
      +   '</div>'
      +   '<div style="padding:14px; overflow:auto;">'
      +     '<div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">'
      +       '<input id="tagManageNewTitle" placeholder="New Tag Name" style="flex:1; min-width:260px; padding:10px 12px; border-radius:12px; border:1px solid rgba(255,255,255,0.14); background:#0d1426; color:var(--text);" />'
      +       '<input id="tagManageNewDisplayName" placeholder="Tag Display Name" style="flex:1; min-width:260px; padding:10px 12px; border-radius:12px; border:1px solid rgba(255,255,255,0.14); background:#0d1426; color:var(--text);" />'
      +       '<button id="tagManageCreate" class="pill" type="button">+ Tag</button>'
      +     '</div>'
    ...
    async function refreshLists(){
      ...
      function pill(t, canEdit){
        ...
        function openEditor(){
          ...
          cancelBtn.className = 'pill off';
          cancelBtn.textContent = 'Cancel';

          cancelBtn.onclick = async () => { await refreshLists(); };
    ...
    if (btnCreate) {
      btnCreate.onclick = async () => {
        ...
        try {
          ...
          alert('Tag create failed: ' + String(e));
          setM('Create failed.');
        }
      };
    }
  ...
  if (chatTagManage) chatTagManage.addEventListener('click', openManageDialog);
  ...
  if (btnCatchUp) btnCatchUp.addEventListener('click', () => {
    ...
    const insert = [
      ...
      '4) Skim Docs repos (if present) so you know what\'s already published:',
      '   - <WORKSPACE_ROOT>/apps/docs/mine (Boss docs repo)',
      '   - /opt/*/apps/docs/mine (team member docs repo on a box)',
      '   - /var/lib/*/apps/docs (state for docs app, if any)',
      '',
      'Rules:',
  ...
  Promise.all([loadBuild(), loadBrand()]).then(async () => {
    ...
    // One-time setup: if identity hasn't been saved yet, prompt the user.
    try {
      const b = await identityLoad();
      const done = b && typeof b.setupDoneAt === 'string' && b.setupDoneAt.trim();
      // Only auto-open when truly uninitialized (empty provider + empty host notes).
      // This avoids nagging older installs that only stored assistantName.
      const uninit = b && !String(b.hostingProvider||'').trim() && !String(b.hostNotes||'').trim();
      if (!done && uninit) {
        identityOpen();
        try { if (identityMsg) identityMsg.textContent = 'One-time setup: add install details.'; } catch {}
      }
    } catch {}
  });
  refresh();
